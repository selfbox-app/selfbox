...
export default function PluginsPage() {
  ...
  const { requestDelete: requestUninstall, dialogProps: uninstallDialogProps, pendingItem: pendingUninstall } = useConfirmDelete<{ id: string; name: string }>({
    onConfirm: (item) => uninstallMutation.mutate({ id: item.id }),
  });
    ...
    <div>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-8">
        {!isAdmin ? (
          <div className="rounded-lg border border-amber-300/40 bg-amber-50 p-4 flex items-start gap-2">
            ...
            <p className="text-sm text-amber-900">
              You can use installed extensions, but only workspace admins can install
              or configure them.
            </p>
          </div>
        ) : null}
                  ...
                  {isAdmin ? (
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={() => requestUninstall({ id: plugin.id, name: plugin.manifest.name })}
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Extension Catalog</h2>
          </div>
          {catalogLoading ? (
            <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">
            ...
            <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">
              No additional extensions available for this workspace.
            </div>
          ) : (
            <div className="grid gap-3">
      ...
      <Dialog open={registerDialogOpen} onOpenChange={setRegisterDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Register In-House Extension</DialogTitle>
            <DialogDescription>
              Paste an extension manifest JSON document. It will become available in this
              workspace catalog.
            </DialogDescription>
          </DialogHeader>
      ...
      <ConfirmDeleteDialog
        ...
        description={pendingUninstall ? `Are you sure you want to uninstall "${pendingUninstall.name}"?` : ""}
