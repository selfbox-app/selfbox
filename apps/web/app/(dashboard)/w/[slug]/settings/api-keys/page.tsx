...
export default function ApiKeysPage() {
  ...
  const { requestDelete: requestRevoke, dialogProps: revokeDialogProps } = useConfirmDelete<string>({
    onConfirm: (id) => revokeKey.mutate({ id }),
  });

  const { requestDelete: requestDeleteKey, dialogProps: deleteKeyDialogProps } = useConfirmDelete<string>({
    onConfirm: (id) => deleteKey.mutate({ id }),
  });
  ...
  return (
    <div>
          ...
          <div className="rounded-lg border bg-card p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Tooltip>
                ...
                <TooltipContent side="top" sideOffset={8}>Copy</TooltipContent>
              </Tooltip>
            </div>
            <Separator />
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <Tooltip>
                    ...
                    <TooltipContent side="top" sideOffset={8}>{showSecret ? 'Hide' : 'Show'}</TooltipContent>
                  </Tooltip>
                  <Tooltip>
                    <TooltipTrigger asChild>
      ...
      <ConfirmDeleteDialog {...revokeDialogProps} title="Revoke API key" description="Are you sure you want to revoke this key? It will no longer be usable." isPending={revokeKey.isPending} />
      <ConfirmDeleteDialog {...deleteKeyDialogProps} title="Delete API key" description="Are you sure you want to permanently delete this key? This action cannot be undone." isPending={deleteKey.isPending} />
    </div>
  );
}
