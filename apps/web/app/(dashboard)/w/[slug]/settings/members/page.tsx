...
export default function MembersPage() {
  ...
  const { requestDelete: requestRemoveMember, dialogProps: removeMemberDialogProps, pendingItem: pendingMember } = useConfirmDelete<{ id: string; name: string }>({
    onConfirm: (item) => removeMember.mutate({ memberId: item.id }),
  });
  ...
  return (
    <div>
              ...
              {invites.map((invite) => (
                ...
                >
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="top" sideOffset={8}>Revoke invite</TooltipContent>
                  </Tooltip>
                </div>
              ))}
            </div>
                  ...
                  {isAdmin && member.role !== 'owner' && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem
                          variant="destructive"
                          onSelect={() => requestRemoveMember({ id: member.id, name: member.userName ?? member.userEmail })}
                        >
                          <UserMinus />
                          Remove
                        </DropdownMenuItem>
      ...
      <ConfirmDeleteDialog
        ...
        title="Remove member"
        description={pendingMember ? `Are you sure you want to remove ${pendingMember.name} from this workspace?` : ""}
