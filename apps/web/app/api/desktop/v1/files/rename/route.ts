...
import { desktopFileRenameSchema } from "@selfbox/common";
import { authenticateDesktopRequest } from "@/server/desktop/auth";
import { desktopRenameFile } from "@/server/desktop/mutations";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { idempotencyKey, ...payload } = desktopFileRenameSchema.parse(
      await request.json(),
    );
    const result = await withIdempotency(
      getDb(),
      { key: idempotencyKey, deviceId: device.id, endpoint: "files/rename" },
      () =>
        desktopRenameFile(getDb(), {
          ...payload,
