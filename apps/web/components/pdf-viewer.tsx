    ...
    >
      <div className="relative rounded-sm overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.08),0_4px_12px_rgba(0,0,0,0.04)]">
        <canvas
          ref={canvasRef}
          className={cn(
            "block bg-card transition-opacity duration-200",
...
export function PDFViewer({
  ...
  const handleFitWidth = useCallback(() => {
    ...
    setScale(Math.max(MIN_SCALE, Math.min(MAX_SCALE, (width - 48) / vp.width)));
  }, [pages]);
  ...
  const handleFitPage = useCallback(() => {
    ...
    const widthScale = (width - 48) / vp.width;
    const heightScale = containerHeight / vp.height;
    setScale(Math.max(MIN_SCALE, Math.min(MAX_SCALE, Math.min(widthScale, heightScale))));
  }, [pages]);
