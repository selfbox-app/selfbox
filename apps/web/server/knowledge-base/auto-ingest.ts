...
export async function ingestFileIntoKB(params: {
  ...
  // Check if the KB extension is active before proceeding
  const [kbPlugin] = await db
    .select({ status: workspacePlugins.status })
    .from(workspacePlugins)
    .where(
      and(
        eq(workspacePlugins.workspaceId, workspaceId),
        eq(workspacePlugins.pluginSlug, "knowledge-base"),
      ),
    )
    .limit(1);

  if (!kbPlugin || kbPlugin.status !== "active") {
    return { status: "skipped", message: "Extension is disabled" };
  }
