...
import { folders, files, workspaces } from "@selfbox/database";
import type { Database } from "@selfbox/database";
import type { StorageProvider } from "@selfbox/storage";
...
export function createPluginStorage(params: {
  ...
  return {
    async upload(uploadParams) {
      ...
      // Emit a `created` sync event so desktop clients pick this up on
      // their next change-feed poll. Without this, plugin-driven uploads
      // are invisible to desktop sync — files sit on the server forever
      // and never come down.
      if (inserted) {
        await recordWorkspaceSyncEvent(db, {
          ...
          eventType: "created",
          payload: createFileSyncPayload(inserted),
        });
      }
