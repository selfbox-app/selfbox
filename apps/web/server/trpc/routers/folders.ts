import { z } from "zod";
import { eq, and, asc, isNull, not, like, sql } from "drizzle-orm";
import { createRouter, workspaceProcedure } from "../init";
import { folders, files } from "@selfbox/database";
import {
  createFolderSchema,
  renameFolderSchema,
  moveItemSchema,
...
import { subtreeDeleteFolder } from "../../desktop/mutations";
...
export const foldersRouter = createRouter({
  ...
  create: workspaceProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await recordWorkspaceSyncEvent(ctx.db, {
        workspaceId: ctx.workspaceId,
  ...
  delete: workspaceProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      await subtreeDeleteFolder(ctx.db, ctx.workspaceId, input.id, ctx.userId);
      invalidateWorkspaceVfsSnapshot(ctx.workspaceId);
      return { success: true };
    }),
});
