...
import { inviteMemberSchema, updateMemberRoleSchema } from "@selfbox/common";
...
export const membersRouter = createRouter({
  ...
  invite: workspaceAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Fetch workspace and inviter info (used for both email and notification)
      ...
      try {
        const { sendEmail } = await import("@selfbox/email");
        const { WorkspaceInviteEmail } =
          await import("@selfbox/email/templates/workspace-invite");

        await sendEmail({
          to: input.email,
          subject: `Join ${wsName} on Selfbox`,
      ...
      try {
        ...
        if (invitedUser) {
          await createNotification({
            ...
            title: `You've been invited to ${wsName}`,
            body: inviter?.name
              ? `${inviter.name} invited you to join ${wsName}`
              : `You've been invited to join ${wsName}`,
            actionUrl: inviteUrl,
            metadata: { workspaceId: ctx.workspaceId, inviteId: invite.id },
          });
        }
      } catch {
