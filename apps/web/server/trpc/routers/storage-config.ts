...
const railwayCredentialsSchema = z.object({
...
const credentialsSchema = z.discriminatedUnion("provider", [
  ...
  railwayCredentialsSchema,
]);

const saveConfigSchema = z
  .object({
    provider: z.enum(["s3", "r2", "vercel", "railway"]),
  ...
  test: workspaceAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const testPath = `.selfbox-connection-test-${Date.now()}`;
