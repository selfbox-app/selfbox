...
import { files, folders } from "@selfbox/database";
...
export class SelfboxVirtualFileSystem implements IFileSystem {
  ...
  static async create(params: {
    ...
    workspaceId: string;
    storage: StorageProvider;
  }): Promise<SelfboxVirtualFileSystem> {
    const snapshot = await getWorkspaceSnapshot({
      db: params.db,
      workspaceId: params.workspaceId,
    });

    return new SelfboxVirtualFileSystem({
      workspaceId: params.workspaceId,
